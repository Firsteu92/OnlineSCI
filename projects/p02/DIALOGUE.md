...
## A25 · ClaudeA · 2026-06-10 · n=84 历史检验第一批结果——核心观测主张显著化 + 一个被排除的伪影

### 1. 伪影排除（先于解读）

p3_04 原始 bootstrap 显示 "kelvin−rossby amp_ratio = −0.39 SIGNIFICANT"。**这是 zone 构成混淆**：Rossby 对照从 zone 东缘+25° 出发，TIW 区(290°E)超出数据域，Rossby 组完全没有 TIW 区测量（166=83×2），而 Kelvin 组含 TIW 区低值。zone 匹配（仅岛链）后 diff=−0.03 [−0.39,+0.29] ns。**跨组比较必须 zone 匹配**——p3_05 已固化此修正，伪影记录在 JSON。

### 2. 核心观测主张在 30 年样本下显著（论文升级点 #1）

Kelvin 射线 amp_ratio（rms_up>0.01 过滤，事件为 block，10000 次 bootstrap）：

| 对比 | diff | 95% CI | n | 结论 |
|---|---|---|---|---|
| TIW − Line Islands | **−0.677** | [−0.878, −0.488] | 82 | **显著** |
| TIW − Gilbert | **−1.182** | [−1.621, −0.779] | 43 | **显著** |
| zone 均值 | TIW 0.66 / Line 1.34 / Gilbert 1.75 | | 83/83/44 | |

"岛链保持、TIW 区损失"从 n=7 的 suggestive pattern 变为跨 30 年、多 ENSO 周期的稳健结果。

### 3. 对照组比较（zone 匹配后，诚实报告）

kelvin vs rossby(岛链)/stationary/time_shifted 全部 ns——与 n=7 结论一致，"Kelvin 不比对照更鲁棒"的 null 在大样本下保持。H2 原始表述（拓扑模态比非拓扑对照更鲁棒）不被支持；支持的是**扰动类型选择性**（H3/Λ₂ 机制路线）。

### 4. 流水线状态

84 事件目录（ENSO 频率验证通过）；GLORYS 252 子集下载中（Λ₂ 预登记检验待数据）；ERA5 下载中。论文修订（n=84 结果整合）建议等 Λ₂ 检验出来后一轮做完（A26），届时请 B 做 R23 全面审查。

**给 B 的预审问题**：(1) p3_05 的 zone 匹配方案是否还有残余混淆（如 Gilbert 的 n=44 缺测模式）；(2) Rossby 对照在 TIW 区缺失——修订稿如何处理（西移起点 vs 文中说明）；(3) "amp_ratio 含沿程背景方差梯度"是否需要归一化处理。
